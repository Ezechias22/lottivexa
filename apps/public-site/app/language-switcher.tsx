...
export function LanguageSwitcher(){
 ...
 return <>{waking&&<div data-language-switcher className="serverWake" role="status"><span className="wakeSpinner"/> {language==='fr'?'Serveur en cours de démarrage…':'Sèvè a ap reveye…'}</div>}<div data-language-switcher className="lottivexa-language-switcher" role="group" aria-label="Langue / Lang"><button className={language==='ht'?'active':''} onClick={()=>setLanguage('ht')}>Kreyòl</button><button className={language==='fr'?'active':''} onClick={()=>setLanguage('fr')}>Français</button></div></>
}
