...
export function LanguageSwitcher(){
 ...
 return <>{waking&&<div data-language-switcher className="serverWake" role="status"><span className="wakeSpinner"/> {language==='fr'?'Serveur en cours de démarrage…':'Sèvè a ap reveye…'}</div>}<div data-language-switcher className="lottivexa-language-switcher" role="group" aria-label="Langue / Lang"><button type="button" className="languageToggle" aria-expanded={open} onClick={()=>setOpen(value=>!value)}>🌐 {language==='fr'?'Français':'Kreyòl'} <span aria-hidden="true">⌄</span></button>{open&&<div className="languageMenu"><button type="button" className={language==='ht'?'active':''} onClick={()=>{setLanguage('ht');setOpen(false)}}>Kreyòl</button><button type="button" className={language==='fr'?'active':''} onClick={()=>{setLanguage('fr');setOpen(false)}}>Français</button></div>}</div></>
}
