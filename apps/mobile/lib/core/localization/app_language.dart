...
class AppLanguage {
  ...
  static Future<void>restore()async{try{final value=await _storage.read(key:_key);current.value=Locale(value=='fr'?'fr':'ht');}catch(_){try{await _storage.delete(key:_key);}catch(_){ }current.value=const Locale('ht');}}
