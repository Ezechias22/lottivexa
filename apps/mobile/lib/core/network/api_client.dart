...
class ApiClient {
  ApiClient(this.session,{String baseUrl=const String.fromEnvironment('API_URL',defaultValue:'https://lottivexa-api.onrender.com')}):dio=Dio(BaseOptions(baseUrl:baseUrl,connectTimeout:const Duration(seconds:75),receiveTimeout:const Duration(seconds:75))){
  ...
  final Dio dio;
  Future<void>warmUp()async{for(var attempt=0;attempt<3;attempt++){try{await dio.get('/api/v1/health',options:Options(extra:{'skipRefresh':true},receiveTimeout:const Duration(seconds:75)));return;}catch(_){if(attempt==2)rethrow;await Future<void>.delayed(const Duration(seconds:4));}}}
