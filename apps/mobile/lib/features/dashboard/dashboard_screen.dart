...
class _State extends State<DashboardScreen> {
  ...
  Future<void> load() async {
    ...
    try {
      final dashboardFuture = widget.api.dio.get<Map<String, dynamic>>(
        '/api/v1/merchants/me/dashboard',
      );
      final drawsFuture = widget.api.dio.get<List<dynamic>>(
      ...
      final openDraws = (drawsResponse.data ?? <dynamic>[])
      ...
      if (mounted) {
        setState(() {
          error = 'Mòd offline: done sou sèvè a poko disponib.';
        });
      }
    } finally {
      if (mounted) {
  ...
  String _money(dynamic value) {
    return 'HTG ${(double.tryParse('$value') ?? 0).toStringAsFixed(2)}';
  }
  ...
  String _drawLabel(Map<String, dynamic> draw, {required bool french}) {
    ...
    final session = RegExp(r'\b(MORNING|MATIN|MATEN)\b').hasMatch(sessionText)
        ? (french ? 'Matin' : 'Maten')
        : RegExp(r'\b(MID|MIDI|MIDDAY|NOON|DAY|JOUR)\b')
                .hasMatch(sessionText)
            ? 'Midi'
            : RegExp(r'\b(EVENING|SOIR|SWA|EVE)\b')
                    .hasMatch(sessionText)
                ? (french ? 'Soir' : 'Swa')
                : null;

    final source = draw['resultAt'] ??
        draw['drawTime'] ??
        draw['closesAt'] ??
        draw['opensAt'];
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (widget.store.pendingCount > 0)
            Card(
              child: ListTile(
                leading: const Icon(Icons.pending_actions),
                title: Text(AppLanguage.tr('Tikè an atant')),
                subtitle: Text(
                  AppLanguage.tr('Yo dwe pase validasyon sèvè a.'),
                ),
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(AppLanguage.tr('Verifye Tikè')),
                ),
              ),
              ...
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(AppLanguage.tr('Gade Rezilta')),
                ),
              ),
            ],
          ),
          Card(
            child: Column(
              children: [
                ListTile(
                  title: Text(
                    AppLanguage.tr('Dènye tikè yo'),
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
                  trailing: TextButton(
                    onPressed: () => context.go('/tickets'),
                    child: Text(AppLanguage.tr('Wè tout')),
                  ),
                ),
                if (recent.isEmpty)
                  ListTile(
                    title: Text(AppLanguage.tr('Pa gen tikè.')),
                  )
                else
          ...
          Card(
            child: Column(
              children: [
                ListTile(
                  title: Text(
                    AppLanguage.tr('Pwochen tiraj yo'),
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
                  trailing: TextButton(
                    onPressed: () => context.go('/new-ticket'),
                    child: Text(AppLanguage.tr('Vann')),
                  ),
                ),
                if (draws.isEmpty)
                  ListTile(
                    title: Text(
                      AppLanguage.tr('Pa gen tiraj ouvè kounye a.'),
                    ),
                  )
                else
                    ...
                    Builder(
                      builder: (context) {
                        ...
                        return ListTile(
                          ...
                          leading: const Icon(
                            Icons.flag_outlined,
                            color: Color(0xff2451c7),
                          ),
                          title: Text(_drawLabel(row, french: french)),
                          subtitle: Text(
                            '${row['game']?['name'] ?? ''}',
                          ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(icon, color: const Color(0xff2451c7)),
