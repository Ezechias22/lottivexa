...
class AdminHubScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final items = <({String permission, String path, String label, IconData icon})>[
      ...
      (permission: 'settings.edit', path: 'results', label: 'Antre rezilta tiraj', icon: Icons.edit_calendar),
    ].where((item) => item.permission.isEmpty || session.hasPermission(item.permission));
    return ListView(padding: const EdgeInsets.all(20), children: [Text(AppLanguage.tr('Administrasyon mobil'), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold)), Text(AppLanguage.tr('Siveyans ak administrasyon tenant lan.')), const SizedBox(height: 16), ...items.map((item) => Card(child: ListTile(leading: Icon(item.icon), title: Text(AppLanguage.tr(item.label)), trailing: const Icon(Icons.chevron_right), onTap: () => context.push(item.path == 'reports' || item.path == 'notifications' ? '/${item.path}' : '/admin/${item.path}'))))]);
  }
}
