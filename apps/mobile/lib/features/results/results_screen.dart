...
class _ResultsState extends State<ResultsScreen> {
  ...
  String drawLabel(Map<String,dynamic>row){final game='${row['game']?['name']??'Lotri'}',draw='${row['drawNumber']??''}';final match=RegExp(r'-(\d{4})$').firstMatch(draw);if(match==null)return '$game · $draw';final hour=int.tryParse(match.group(1)!.substring(0,2));if(hour==null||hour>23)return '$game · $draw';final period=hour<12?'Maten':hour<17?'Midi':hour<21?'Swa':'Lannuit';return '$game · ${AppLanguage.tr(period)} · $draw';}
