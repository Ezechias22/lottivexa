...
class _State extends State<SettingsScreen>{
  ...
  late final device=TextEditingController(text:widget.runtime.deviceId);
  ...
  Future<void>sync()async{setState(()=>busy=true);try{await widget.runtime.setDeviceId(device.text);await widget.runtime.recover();if(mounted)setState(()=>message='Sync fini. ${widget.runtime.pendingCount} transactions ak ${widget.runtime.printer.pendingCount} prints rete.');}catch(error){if(mounted)setState(()=>message='Sync pa fèt: $error');}finally{if(mounted)setState(()=>busy=false);}}
