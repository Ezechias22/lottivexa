...
class _State extends State<SettingsScreen>{
  ...
  Future<void>sync()async{setState(()=>busy=true);try{await widget.runtime.setDeviceId(device.text);await widget.runtime.recover();if(mounted)setState(()=>message='Sync fini. ${widget.runtime.pendingCount} ap tann, ${widget.runtime.store.rejectedCount} rejte; ${widget.runtime.printer.pendingCount} enpresyon ap tann.');}catch(error){if(mounted)setState(()=>message='Sync pa fèt: $error');}finally{if(mounted)setState(()=>busy=false);}}
